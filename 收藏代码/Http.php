...
class Http {
    ...
    public static function post($url, $data, $timeout = 100, $header = null)
    {
        ...
        if(!empty($errno)){
            Logger::info('curl返回结果:'.$url.'::'.$errno,array(),"Curl");
            Logger::info('curl返回结果:'.$url.'::'.$error,array(),"Curl");
        }
    ...
    public static function get($url, $data, $timeout = 30, $header = null){
        $result = self::request($url, 'GET', $data, $header, $timeout);
        return $result;
    }
    ...
    public static function request($url, $mode, $params = '', $needHeader = false, $timeout = 10)
    {
        ...
        if ($needHeader) {
            ...
            if(!is_bool($needHeader)){
                curl_setopt($curlHandle, CURLOPT_HTTPHEADER, $needHeader);
            }
        }
